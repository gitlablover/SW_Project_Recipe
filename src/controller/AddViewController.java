...
public class AddViewController {
	...
	public void abEvent(ActionEvent event) {
		if (inName.equals("")) {
			Alert alert = new Alert(AlertType.WARNING, "Name and number can't be nothing!", ButtonType.OK);
			alert.showAndWait();
		} else {
	...
	public void cbEvent(ActionEvent event) {
		if (name.equals("")) {
			Alert alert = new Alert(AlertType.WARNING, "Name can't be nothing!", ButtonType.OK);
			alert.showAndWait();
		} else if (inCounter == 0) {
			Alert alert = new Alert(AlertType.WARNING, "Ingredient can't be nothing!", ButtonType.OK);
			alert.showAndWait();
		} else {
			...
			if (result.get() == ButtonType.YES) { // User authorizes the action.
				Cookbook.recipeList.add(new Recipe(name, preTime, cookTime, instruction));
				for (int i = inId; i > inId - inCounter; i--) {
					Cookbook.recipeList.get(Cookbook.recipeList.size() - 1).addIngredient(Cookbook.userIn.get(i));
				}
				Cookbook.nowRecipe = Cookbook.recipeList.get(Cookbook.recipeList.size() - 1);

				try {
					Cookbook.nowRecipe.setId(RecipeDAO.createNewRecipe(Cookbook.nowRecipe));
